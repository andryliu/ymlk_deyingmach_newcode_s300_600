...
int can_sendframe(const int sockfd, const byte* data, const int count)
{
#ifndef WIN32
...
int can_recvframe(const int sockfd, byte* buf, const int count, const int timeout_ms)
{
...
// 每调用一次本函数，帧序号自加1
void can_printframe(const __u32 frame_id, const char *data, const __u8 len, 
    const bool extended, 
    const bool ok_flag, 
    const bool sendflag)
{
  ...
  //  set_can_filter();
    ...
    *pframeno = 1;
    ...
    printf("children\n");
        ...
        {
            ret = can_sendframe(send_socket_fd, (char *)&sendframe, sizeof(sendframe));
    ...
    printf("father\n");
        ...
        while (run)
        {
            ...
            ret = can_recvframe(recv_socket_fd, precvframe, can_frame_len, 5 * 1000);
            if (ret > 0)
            {
