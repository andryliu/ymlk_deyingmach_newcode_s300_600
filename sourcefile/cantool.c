...
int  recv_socket_fd = -1;
S_CanFrame sendframe, recvframe;

char CanReState[3]={0};
extern pthread_mutex_t mutex_canlock;
static int   *pframeno = NULL;
static int   port = 0;
static int   bitrate = 1000 * 1000;
 unsigned int send_frame_id  = 0;
static unsigned int send_frame_freq_ms = 250;
static int   send_frame_times   = 0;
static unsigned char  send_frame_data[8] = {0x00, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
static unsigned char  dlc = 8;


static bool  extended_frame         = 1;
static bool  send_frame_id_inc_en   = 0;// 帧ID每发送一帧递增
static bool  send_frame_data_inc_en = 0;// 发送数据每发送一帧递增
static bool  lp = 0;// 本地环回使能开关
static bool  run = 0;
...
#define TX_QUEUE_LEN		4096 // 使用足够多的发送缓存
	...
	sprintf(l_c8Command, "ip link set can0 type can bitrate %d ", bitrate);
	system(l_c8Command);
...
#ifndef WIN32
	pthread_mutex_lock(&mutex_canlock);
    int ret = write(sockfd, (const char*)data, count);
	pthread_mutex_unlock(&mutex_canlock);
...
#ifndef WIN32
    read(sockfd, (char*)buf, count);
#else
   recv(sockfd, (char*)buf, count, 0);
#endif
